...
def get_bert_layer_local_spec(
    ...
    """Return a Bert TransformerLayer ModuleSpec that uses Megatron-Core components.

    When `num_experts` is not None, the MLP submodule will be a MoE implementation
    (via `get_mlp_module_spec`).
    """
...
# Backwards-compatible alias: keep the old name for callers that import it.
bert_layer_local_spec = get_bert_layer_local_spec()
